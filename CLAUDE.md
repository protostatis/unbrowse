...
1. **TLS fingerprint (JA3/JA4)** — `wreq`, not `reqwest`. Mimics Chrome's BoringSSL cipher order, extensions, ALPN. Set at connection time, can't be added later.
2. **HTTP/2 frame ordering** — Akamai's H2 hash. Also handled by `wreq`.
...
- `__host_fetch` — HTTP via `wreq` with active stealth profile
...
- `~/Projects/sky-search/client/wasm/dom.js` (731 LOC)
- `~/Projects/sky-search/client/wasm/shims.js` (937 LOC) — fetch/XHR/timers/URL/localStorage stub. **Will need stealth additions** (canvas/WebGL/audio forging, navigator patches) — sky-search doesn't have these because the parent browser provides them.
- `~/Projects/sky-search/client/wasm/intel.js` (161 LOC)
...
2. ✅ **Fetch + parse** — `wreq` navigate with Chrome profile, html5ever parse, DOM seeded into JS. TLS+H2 fingerprint verified against `tls.peet.ws`.
...
5c. ✅ **Phase A observability** — `Session.nav_counter` mints `nav_<n>` per `navigate_with()`. NDJSON event taxonomy (all carry `schema_version: 1` + `navigation_id`): `navigation_started` (after DOM seed, paired-with-trace invariant), `script_decision` per external script with `action: skip|queued|fetch_failed` (+ `reason: "blocklist"` when skipped), `script_executed` per evaled script with `duration_us` + `error` + `interrupted`, `policy_trace` summary at end (always emitted, paired with start). `report_outcome` RPC method validates `navigation_id` against issued set, requires `success: bool`, validates `task_class` against the `extract|query|click|form|visual` enum, emits `outcome_reported`. Drivers join outcomes back to traces via `navigation_id`. v0 has no posterior store yet — that's Phase B. Legacy `policy_blocked` event still fires for one cycle (drop in follow-up). See `docs/probabilistic-policy.md` §4.5 for the outcome protocol spec.
...
7. ✅ **Interactivity (v1)** — `src/js/interact.js` provides `__byRef`, `__click`, `__type`, `__formData`. RPC methods `click` / `type` / `submit`. Click on `<a href>` auto-follows (resolves relative URLs against current page, then navigates). Submit supports GET and `application/x-www-form-urlencoded` POST; multipart is not supported. Verified end-to-end: HN → `type wreq` into `input[name=q]` → `submit` → landed on `https://hn.algolia.com/?q=wreq`.
8. (skipped 8/9/10 numbering)
11a. ✅ **Cookie jar** — `CookieJar` struct in `main.rs` implements `wreq::cookie::CookieStore`. Cookies in response `Set-Cookie` headers auto-populate; cookies are auto-sent on subsequent requests to matching domains. RPC methods `cookies_set` (object or string form, accepts `{name, value, domain, path?, secure?, http_only?, url?}`), `cookies_get`, `cookies_clear`. Persistence (file save/load) is the agent driver's responsibility — the binary is stateless. **The killer use case:** solve a PerimeterX/Datadome/Cloudflare challenge once in real Chrome, copy the clearance cookie via DevTools, paste into a `cookies_set` call, run unbrowser against the protected site for the cookie's lifetime. End-to-end verified against `zillow.com/homes/for_rent/`: 403+captcha without cookies → 200+626KB rentals page with cookies replayed.
12. ✅ **MCP server mode** — `unbrowser --mcp` enters Model Context Protocol mode: JSON-RPC 2.0 on stdio with `initialize` / `tools/list` / `tools/call` / `ping`. RPC methods are exposed as MCP tools (everything except `close` — host manages lifecycle). JSON schemas inline for tool discovery. Result content returned as pretty-printed JSON in a single `text` content item. Errors set `isError: true`. Verified via Python driver: handshake → tools/list → tools/call navigate → tools/call query with sibling combinator → bad-tool-name returns isError=true.
...
- `navigate {url, exec_scripts?, include_ascii?}` — `wreq` GET with the active Chrome emulation profile; parses HTML with html5ever; seeds the JS DOM. Returns `{status, url, bytes, blockmap, challenge}`. The blockmap is the one-shot orientation payload (v2: compressed, no summary, no main_headings, sparse samples, RLE structure grouping). The `challenge` field is `null` on the happy path; on bot-detection responses it's `{vendor, status, clearance_cookie, hint}` so the agent can react (e.g. escalate to real Chrome). Detected vendors: PerimeterX, Cloudflare, Datadome, Akamai BMP, Imperva. Pass `include_ascii: true` for the human-readable ASCII grid; default false. CLI output is pretty-printed by default; use `--compact` or `--json` for single-line output.
...
- `submit {ref}` — gather form field values and navigate to the resolved action URL. Supports GET and `application/x-www-form-urlencoded` POST; checked checkbox/radio values are serialized. Multipart is not supported.
...
**BlockMap shape (v2):** returns `blockmap_version: 2` with a compressed wire format targeting ~500-4K tokens per page. Key fields:

```
{ blockmap_version, title,
  structure: [{role, ref, ident, counts, repeat?}],  // repeat = collapsed duplicates
  headings: [{level, text, ref, chrome}],             // chrome=true means inside header/nav/footer/aside
  interactives: {
    links, buttons,                 // total counts
    link_samples: [{ref, text, href, aria_label?, title?, role?}],   // top 24, null fields dropped
    button_samples: [{ref, text, type, aria_label?, title?, role?, matches?}], // top 12, grouped
    inputs, forms                   // form serialization
  },
  density: {tables, td, li, json_scripts, script_tags, body_text_chars, likely_js_filled, ...},
  selectors: {data_testid, aria_label, role},
  ascii: "<multiline>"            // default off — pass include_ascii:true to get it
}
```

The `ascii` field is gated behind `include_ascii: true` (default false). `structure`/`headings`/`interactives`/`density`/`selectors` are what the agent uses to plan queries. Repeated structure entries (same role+ident+counts shape) emit up to 3 examples with `repeat: N` on the last — e.g. Yahoo Finance's 27 story cards collapse to 3 entries + `repeat: 27`. Link samples capped at 24, button samples at 12 (repeated labels grouped as `"Follow" ×5`). Landmarks (`header, nav, main, aside, footer, article, section`) are detected first; if none, the walker falls back to significant top-level children of `<body>`. Navigate response headers contain only `content-type` + `location` (full headers ~1-8K tokens of CSP/cookie/ad-tech noise were stripped).
...
- `wreq` is the load-bearing crate for stealth. If it goes unmaintained, fall back to `boring` (BoringSSL) + custom `hyper` configuration.
